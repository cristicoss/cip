      ...
      if (!jobID) {
        ...
        const displayJobs = function (jobs) {
          if (jobs.length > 0) {
            jobs.forEach((job) => {
              const html = `
            <a href="https://cip-new.webflow.io/retailexperts?jobid=${job.id}" class="job_title-wrapper w-inline-block">
        ...
        displayJobs([...retailExperts]);
      }

      backBtn.addEventListener("click", function () {
        window.location.href = `https://cip-new.webflow.io/retailexperts`;
      });

      btnSpontan.addEventListener("click", function () {
        window.location.href = `https://cip-new.webflow.io/retailexperts/?jobid=109507`;
      });
    };
